...
$(document).ready(function () {
  all_customers = $('#all_customers').val();
  if(all_customers){ 
    all_customers = JSON.parse(all_customers);
  }
    if(segments[4] != 'create-package' && segments[3] != 'create-customer-package' && segments[4] != 'print-packages-label'){
      homeStatus =$('#homePageStatus').val();
      $('#homePageStatus').val('');
...
$(document).on('change', '#origin', function() {
    ...
    $('#origin_name').val(selectedText);
     rate = 0; 
    if (origin && origin.trim()) {
        rate = Number($('option:selected', this).attr('rate')); 
    }  
    calculateGrandTotal();
});
...
function calculateGrandTotal() {
      kg = Number($('#kg').val()); 
      console.log(kg,'kg');   
      let isInsured = $('input[name="is_insured"]:checked').val();
    if (isNaN(kg) || kg <= 0 || rate <= 0) {
        $('#grand_total').val(0);
        return;
    }
    else{
      let grand_total = kg * rate;
      console.log("Base Grand Total:", grand_total); 
      if (isInsured === 'insured') {
        let tenPercent = grand_total * 0.10;  
        console.log("10% of Grand Total (before rounding):", tenPercent);
        tenPercent = roundToTwoDecimals(tenPercent);  
        grand_total += tenPercent;
    }  
...
$('input[name="is_insured"]').on('change', function() {
  if ($(this).val() === 'insured' && $(this).is(':checked')) {
      console.log('Insured option selected');
  } else if ($(this).val() === 'not-insured' && $(this).is(':checked')) {
      console.log('Not Insured option selected');
  }
  calculateGrandTotal(); 
});

$(document).on('click','#get_all_package',function(){
    fetchPackages(false)
})


function fetchPackages(week_flag=true) {
  var package_status = $('#package_status').val();
  var url = '/admin/getPackages'; 
  if (!is_web.is_web) {
      week_flag=false;
  ...
  let currentSearch = $('#customSearchInput').val() || '';
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      packages.forEach(element => {
        ...
        $('#packageTable tbody').append(`
                    <tr>
                        ...
                        <td>     
                            <div ${is_web.is_web ? `data-toggle="modal" data-target="#requestStatus"` : ''} 
                                waybill="${element.waybill}" 
                                id="${element.id}" 
                                data-value="${element.status}" 
                                class="textMiddle change_status btn-status-${element.id} status-box ${className}">
      ...
      if (currentSearch) {
        packageTable.search(currentSearch).draw();
      }
...
$(document).on('click','.changePaymentStatus',function(){
  var id = $(this).attr('id');
  var paymentStatus = $(this).attr('payment-status') == 1? Lang.get(`fields.mark_as_paid`): Lang.get(`fields.mark_as_unpaid`);
  var thisRef = $(this);
  if(id && is_web.is_web){
    ...
    $.ajax({
      type: 'POST',
      url: '/admin/change-package-payment-status',
      data:{
        id:id, 
      ...
      success: function (response) { 
        ...
        if(response.status == 'success'){ 
          $(document).find(`.statusTd-${id}`).html(thisRef.attr('payment-status') == 1? Lang.get(`fields.unpaid`): Lang.get(`fields.paid`));
          thisRef.attr('payment-status', thisRef.attr('payment-status') == 1?0:1);
          thisRef.text(Lang.get(paymentStatus))
          $("#notifDiv").fadeIn();
          $("#notifDiv").css("background", "green");
          $("#notifDiv").text("Payment Status Updated successfully");
          setTimeout(() => {
          ...
          thisRef.text(thisRef.attr('payment-status') == 1? Lang.get(`fields.mark_as_unpaid`): Lang.get(`fields.mark_as_paid`)); 
          $("#notifDiv").fadeIn();
...
$(document).on('click','#search_package',function(){
  var package_status = $('#package_status').val(); 
  $(this).attr('disabled',true);
  $(this).text(Lang.get('fields.processing'));
  fetchPackages(false);
  setTimeout(() => {
    $(this).text(Lang.get('fields.search'));
    $(this).attr('disabled',false); 
  }, 1000); 

});
$(document).on('click','.print_package_label',function(){
   
    var waybill = $('#tracking_waybill').val();
    var thisRef = $(this);
    if(!waybill || !waybill.trim()){
        $("#notifDiv").fadeIn();
        $("#notifDiv").css("background", "red");
        $("#notifDiv").text("Please enter waybill");
        setTimeout(() => {
            $("#notifDiv").fadeOut(); 
          }, 1500);
    }else{  
      waybill = waybill.replace(/(\d)(\s)(\d)/, '$1-$3');
      console.log(waybill) 
      let url = `/admin/print-packages-label/${waybill}`;
      ...
      return ;
        ...
        $.ajax({
            type: "GET",
            url: `/admin/print-packages-label/${waybill}`,
            data: {  
              waybill:waybill 
            },
            cache: false,
            success: function (response) {
              thisRef.attr("disabled", false);
              thisRef.text(Lang.get('fields.print_label')); 
              if (response.status == "success") { 
                $('#tracking_status').val('')   
                ...
                $("#notifDiv").fadeIn();
                $("#notifDiv").css("background", "red");
                $("#notifDiv").text("Failed to print label at the moment");
                setTimeout(() => {
                  $("#notifDiv").fadeOut();
                }, 3000);
              }
...
let debounceTimer;
$(document).on('keyup', '#tracking_waybill', function () {
...
$(document).on("click", ".update_status", function () {
    ...
    var thisRef = $(this);
  var  url =  "/admin/change-package-status";
  var isTracking = false;
    if(thisRef.hasClass('isTracking')  ){
        isTracking = true;
        status = $('#tracking_status').val();
        id     = $('#tracking_waybill').val().trim();
        url   = '/admin/change-package-tracking-status'
    }
    console.log(status,id);

    if(id && status){
        thisRef.attr("disabled", "disabled");
        thisRef.text(Lang.get('fields.updating'));
        $.ajax({
            type: "POST",
            url: url,
            data: {
            ...
            success: function (response) {
              thisRef.attr("disabled", false);
              thisRef.text(Lang.get('fields.update_button'));
              if (response.status == "success") {
                if(isTracking){
                    $('#tracking_waybill').val('') 
                  }else{
                    fetchPackages();
                  }
                $("#notifDiv").fadeIn();
                ...
                $("#notifDiv").fadeIn();
                $("#notifDiv").css("background", "red");
                $("#notifDiv").text("Failed to update status at the moment");
                setTimeout(() => {
                  $("#notifDiv").fadeOut();
                }, 3000);
              }
...
$(document).on("click", ".change_status", function () {
    ...
    var id = $(this).attr("id");
    var lesson_title = $(this).attr("waybill");
    $(".package-title").empty();
    $(".package-title").html(Lang.get('fields.actions')+": ");
    $(".package-title").append(`<span>${lesson_title}</span>`);
    $(".update_status").attr("id", id); 
      $(`#${status}`).prop("checked", true); 
  });
$(document).on('click','.delete_package_record',function(){
    const id = $(this).attr('id');
    $(".confirm_delete").attr("id", id);
    $(".confirm_delete").removeClass('confirmStatusChangeEmp');
...
$(document).on("click", ".confirm_delete", function () { 
    ...
    if(id){
        $.ajax({
            ...
            success: function (response) {
              ...
              if (response.status == "success") {
                ...
                $("#notifDiv").fadeIn();
                $("#notifDiv").css("background", "green");
                $("#notifDiv").text(" Package Deleted successfully");
                setTimeout(() => {
                  $("#notifDiv").fadeOut();
                }, 3000);
              } else {
                $("#notifDiv").fadeIn();
                $("#notifDiv").css("background", "red");
                $("#notifDiv").text("Failed to delete Package at the moment");
                setTimeout(() => {
                  $("#notifDiv").fadeOut();
                }, 3000);
              }
        ...
        $('#notifDiv').fadeIn();
        $('#notifDiv').css('background', 'red');
        $('#notifDiv').text('Invalid Package Record');
        setTimeout(() => {
            $('#notifDiv').fadeOut();
        }, 3000);
        return;
...
$(document).on('click', '#savePackage', function () {
    ...
    var url = is_web.is_web?'/admin/save-package':"/save-customer-package"
 var comments = CKEDITOR.instances['comments'].getData();
    $('#savePackageForm').ajaxSubmit({
        type: "POST",
        url: url,
        cache: false,
        ...
        success: function (response) {
            ...
            if (response.status == "success") { 
                if(response.barcodeUrl && response.barcodeUrl != null){
                    packageSaved = response.package; 
                    if(packageSaved){
                        $(".modal-title").text("Print");
                        $(".modal-custom-text").html(
                          "Do you want to print this label?"
                        );
                        $('.confirm_delete').addClass('print_label');
                        $('.confirm_delete').removeClass('confirm_delete');
                        $("#hidden_btn_to_open_modal").click(); 
                        $('#notifDiv').text(response.msg); 
                                      $('#notifDiv').fadeIn();
                                      $('#notifDiv').css('background', 'green');
                    ...
                    $('#notifDiv').css('background', 'green');
                    setTimeout(() => {
                        window.location.href = is_web.is_web?'/admin/packages':"customer-packages";
                        $('#notifDiv').fadeOut();
                    }, 3000);
                }
                ...
                $('#notifDiv').fadeIn();
                $('#notifDiv').css('background', 'red');
                $('#notifDiv').text('Failed to save Package at the moment');
                setTimeout(() => {
                    $('#notifDiv').fadeOut();
                }, 3000);
            }
...
$(document).on("click", ".print_label", function () { 
    window.location.href = is_web.is_web?`/admin/package-print-label/${packageSaved.id}`:`/print-customer-packages-label/${packageSaved.id}`;
    $('.btn-close').click();
    setTimeout(()=>{
      window.location.href = is_web.is_web?'/admin/packages':'customer-packages';
    },3000)
...
$(document).on("click", ".cancel_delete_modal", function (){
    if(segments[4] != 'packages')
      window.location.href = is_web.is_web?'/admin/packages':'customer-packages';
});
$(document).on('click','.openSidebarForPackage',function(){
...
if(id){
  let filteredPackages = packages.find(x => x.id == id);
  if(filteredPackages){
  $('#origin').val(filteredPackages.origin_id).trigger('change');
  $("#kg").val(filteredPackages.kg);
  $("#cbm").val(filteredPackages.cbm);
  $("#grand_total").val(filteredPackages.grand_total);  
  openSidebar();
  }
}else{
  $('#notifDiv').fadeIn();
...
function getClientDetails(suit){
   
  if(is_web.is_web){
    $.ajax({
      type: 'GET',
      url: `/admin/getPackageClient/${suit}`,
      success: function (response) {  
          if(response.client){ 
              ...
              if(isFirst == 1){
                  $('.client_name_div').empty().append(`
                      <strong>Client Name:</strong> ${response.client.first_name ?? ''} ${response.client.last_name ?? ''}
                  `);
                  isFirst = isFirst+1;  
              }
          }else{
          ...
          $('#savePackage').attr('disabled',false)
          $('#client_id').val('');
          $('#first_name').val('');
          $('#last_name').val('');
      }
...
$(document).on('change','#customer-select-id',function(){
  var customer_select_id = $(this).val();
  if(customer_select_id){
    filterCust = all_customers.find(x=>x.id == customer_select_id);
    if(filterCust){
      $('#suite').val(filterCust.suite)
      $('#first_name').val(filterCust.first_name)
      $('#last_name').val(filterCust.last_name)
      $('#client_id').val(filterCust.id);
    }else{
      $('#suite').val('')
      $('#first_name').val('')
      $('#last_name').val('')
...
$(document).on('input', '#suite', function() {
  if(is_web.is_web){ 
  clearTimeout(typingTimer);
  
     
    $('#first_name').val('Loading...');
    $('#last_name').val('Loading...');   
    $('#client_id').val('');
...
$(document).on('click', '#saveSideBarBtn', function () {
   let ref = $(this);
   if(is_web.is_web){
    $(ref).attr('disabled', true); 
    $(ref).text(Lang.get('fields.processing'));
  
    $('#savePackageSidebar').ajaxSubmit({
        type: "POST",
        url: '/admin/save-package-sidebar',
        cache: false,
        success: function (response) {
            $(ref).attr('disabled',false);
