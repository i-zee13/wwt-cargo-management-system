...
function fetchRates(){
    ...
    $.ajax({
        ...
        success: function (response) { 
            ...
            console.log(rates);
            $('.rate_list').append(`
            <table class="table table-hover dt-responsive nowrap" id="ratesTable" style="width:100%;">
                <thead>
                    <tr>
                        ...
                        <th>${Lang.get('fields.rate')}</th>    
                        <th>${Lang.get('fields.additional')}</th>    
            ...
            rates.forEach(element => {
                ...
                $('#ratesTable tbody').append(`
                    ...
                    <td>${element['id']}</td> 
                        ...
                        <td>${element['type']??''}</td> 
                        <td>${element['origin_name']??''}</td> 
                        <td>${element['rate']??''}</td> 
                        <td>${element['additional'] != null ? element['additional'] :'0.00'}</td> 
                        <td>
                        <button id="${element['id']}" class="btn btn-outline-primary openDataSidebarForAddingRate">
            ...
            $('.rate_list').fadeIn();
           var ratesTable = $('#ratesTable').DataTable({
            responsive: true,
            searching: true,
            lengthChange: false,
            info: false,
            ...
            $("#customSearchInput").on("keyup", function () {
                ratesTable.search(this.value).draw();
              });
...
$(document).on("click", ".confirm_delete", function () { 
    ...
    if(id){
        $.ajax({
            ...
            success: function (response) {
              ...
              if (response.status == "success") {
                ...
                $("#notifDiv").text(" Freight Rate Deleted successfully");
                setTimeout(() => {
                  $("#notifDiv").fadeOut();
                }, 3000);
              } else {
                ...
                $("#notifDiv").text("Failed to delete  Freight Rate at the moment");
                setTimeout(() => {
                  $("#notifDiv").fadeOut();
                }, 3000);
              }
        ...
        $('#notifDiv').fadeIn();
        $('#notifDiv').css('background', 'red');
        $('#notifDiv').text('Invalid  Freight Rate Record');
        setTimeout(() => {
            $('#notifDiv').fadeOut();
        }, 3000);
        return;
...
$(document).on('click', '.openDataSidebarForAddingRate', function () {
    $('#dataSidebarLoader').hide();
    $('#saveRateForm')[0].reset();  
    ...
    if (id && id.trim() != '') {
        $('#opp_id').val(id);
        rate = rates.find(x => x.id == id);
        if (rate) { 
            $('#branch').val(rate.branch_id||'').trigger('change')
            $('#origin').val(rate.origin_id||'').trigger('change');
            $('#rate').val(rate.rate||'')  
            $('#rate_type').val(rate.type||'').trigger('change')
            $('#client_id').val(rate.client_id||'').trigger('change')
            $('#additional').val(rate.additional||'');
        }
    } else {
        $('#opp_id').val(''); 
    }
    ...
    $('#saveRateForm').ajaxSubmit({
        ...
        success: function (response) {
            ...
            fetchRates();
                ...
                $('#notifDiv').fadeIn();
                $('#notifDiv').css('background', 'red');
                $('#notifDiv').text('Failed to save freight rate at the moment');
                setTimeout(() => {
                    $('#notifDiv').fadeOut();
                }, 3000);
            }
