...
function fetchOrigins(){ 
    ...
    $.ajax({
        type: 'GET',
        url: '/admin/getOrigins',
        success: function (response) { 
            origins = response.origins;
            console.log(origins);
            $('.origins_body').append(`
            <table class="table table-hover dt-responsive nowrap" id="originsTable" style="width:100%;">
                <thead>
                    <tr>
                        <th>${Lang.get('fields.id')}</th>
                        <th>${Lang.get('fields.origin_name')}</th>    
            ...
            $('#originsTable tbody').empty();
            origins.forEach(element => {

                $('#originsTable tbody').append(`
                    <tr>
                    <td>${element['id']}</td> 
                        ...
                        <td>
                        <a href="/admin/create-origin/${element.id}" id="${element['id']}" class="btn btn-outline-primary openDataSidebarForAddingClient">
                        <svg width="10" height="13" viewBox="0 0 10 13"
                         ...
                         <button title="Delete"  type="button" id="${element['id']}" class="btn btn-outline-primary   delete_origin_record">
            ...
            $('.origins_body').fadeIn();
           var originsTable = $('#originsTable').DataTable({
            responsive: true,
            searching: true,
            lengthChange: false,
            info: false,
            ...
            $("#customSearchInput").on("keyup", function () {
                originsTable.search(this.value).draw();
              });
...
$(document).on('click','.delete_origin_record',function(){
...
$(document).on("click", ".confirm_delete", function () { 
    ...
    if(id){
        $.ajax({
            ...
            success: function (response) {
              ...
              if (response.status == "success") {
                ...
                $("#notifDiv").text(" Origin Deleted successfully");
                setTimeout(() => {
                  $("#notifDiv").fadeOut();
                }, 3000);
              } else {
                ...
                $("#notifDiv").text("Failed to delete Origin at the moment");
                setTimeout(() => {
                  $("#notifDiv").fadeOut();
                }, 3000);
              }
        ...
        $('#notifDiv').fadeIn();
        $('#notifDiv').css('background', 'red');
        $('#notifDiv').text('Invalid Origin Record');
        setTimeout(() => {
            $('#notifDiv').fadeOut();
        }, 3000);
        return;
...
$(document).on('click', '#saveOrigin', function () {
    var dirty = false;
    var ref = $(this);
    var count = 1;
    ...
    $('#saveOriginForm').ajaxSubmit({
        type: "POST",
        url: '/admin/save-origin',
        cache: false,
        success: function (response) {
            ...
            if (response.status == "success") { 
                ...
                $('#notifDiv').css('background', 'green');
                setTimeout(() => {
                    window.location.href = '/admin/origins';
                    $('#notifDiv').fadeOut();
                }, 3000);
                ...
                $('#notifDiv').fadeIn();
                $('#notifDiv').css('background', 'red');
                $('#notifDiv').text('Failed to save Origin at the moment');
                setTimeout(() => {
                    $('#notifDiv').fadeOut();
                }, 3000);
            }
