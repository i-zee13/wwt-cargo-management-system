              ...
              element.status == 'retired' 
                ...
                $('#packageTable tbody').append(`
                    ...
                    <td>${element['waybill']}</td> 
                        ...
                        <td>${(element['client_suite'] ?? '') + ' ' + (element['first_name'] ?? '') + ' ' + (element['last_name'] ?? '')}</td>
...
$(document).on('click','.print_package_label',function(){
    ...
    if(!waybill || !waybill.trim()){
        ...
        $.ajax({
            type: "GET",
            url: `/admin/print-packages-label`,
            data: {  
              waybill:waybill 
            },
            cache: false,
        ...
        $.ajax({
            ...
            success: function (response) {
              ...
              if (response.status == "success") {
                if(isTracking){
                    $('#tracking_status').val('')
                    $('#tracking_waybill').val('').trigger('change')
                  }else{
                    fetchPackages();
                  }
                $("#notifDiv").fadeIn();
